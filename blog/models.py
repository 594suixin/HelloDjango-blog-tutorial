...
class Category(models.Model):
    ...
    CharField指定了分类名name的数据类型，CharField是字符类型，
...
class Post(models.Model):
    ...
    # 且自Django2.0以后，ForeignKey必须传入一个on_delete参数用来指定当关联的。
    # 数据被删除时，被关联的数据的行为，我们这里假定当某个分类被删除时，该分类下全部文章也同时被删除。
    # 因此使用models.CASCADE参数，意为级联删除。
    # 而对于标签来说，一篇文章可以有多个标签，同一个标签下也有可能有多篇文章，所以我们使用ManyToManyField，表明这是多对多的关联关系。
